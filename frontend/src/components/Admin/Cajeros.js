...
import { gettingCajeros, updatingCajeros } from "../services/Landing.service"

const validationSchema = Yup.object({
    nombre: Yup.string().required("El nombre debe ser capturado"),
    password: Yup.string().required("El password debe ser capturado")
})
...
function Cajeros() {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await gettingCajeros(params)
            if (!response.success) {
                console.error("Error fetching cajeros:", response.error)
                return
            }
            //console.log("response:", response.cajeros)
            setData(response.cajeros)
    ...
    const postData = async cajeros => {
        console.log("cajeros:", cajeros)
        try {
            const response = await updatingCajeros(cajeros)
            if (response.success) {
                console.log('Cajeros actualizado correctamente')
            } else {
                console.error('No se pudo actualizar el cajeros:', response.error)
            }
    ...
    const edit_handled = row => {
        formik.values.id_cajero = row.id_cajero
        formik.values.nombre = row.nombre
        formik.values.password = row.password
  ...
  return (
        <Page title="Landing Page">
            <h4 className="pt-2 pb-3">ABC Cajeros</h4>
            <div className="gap-2">
                <div>
                    ...
                    <tbody>
                        {data.map(row => (
                            <tr key={row.id_cajero}>
                                <td className="align-content-center">{row.id_cajero}</td>
                                <td className="align-content-center">{row.nombre}</td>
                                <td className="align-content-center text-center">{row.password}</td>
            ...
            <>
                <Modal size="lg" show={show} onHide={handleClose} backdrop="static" keyboard={false} centered>
                    <Modal.Header closeButton>
                        <Modal.Title>Cajero # {formik.values.id_cajero === 0 ? "nuevo" : formik.values.id_cajero}</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form>
                            <Row>
                            ...
                            <Row>
                                <Col>
                                    ...
                                    <FloatingLabel label="Password" className="mb-3">
                                        <Form.Control type="text" placeholder="Password" id="password" name="password" onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.password} />
