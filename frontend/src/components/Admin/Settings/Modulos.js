...
import { gettingModulos, postingModulos } from "../../services/Settings.service"
...
function Modulos() {
      ...
      const fetchData = async () => {
          ...
          try {
              const response = await gettingModulos(params)
              if (!response.success) {
                  console.error("Error fetching modulos:", response.error)
                  return
              }
              //console.log("response:", response.modulos)
              setData(response.modulos)
      ...
      const postData = async modulos => {
          console.log("modulos:", modulos)
          try {
              const response = await postingModulos(modulos)
              if (response.success) {
                  console.log('Modulos actualizado correctamente')
              } else {
                  console.error('No se pudo actualizar el modulos:', response.error)
              }
      ...
      const edit_handled = row => {
          formik.values.id_modulo = row.id_modulo
  ...
  return (
    <Page title="ABC Modulos" mt={1}>
            <h4 className="pt-2 pb-3">ABC Modulos</h4>
            <div className="gap-2">
                <div>
                    ...
                    <tbody>
                        {data.map(row => (
                            <tr key={row.id_modulo}>
                                <td className="align-content-center">{row.id_modulo}</td>
            ...
            <>
                <Modal size="lg" show={show} onHide={handleClose} backdrop="static" keyboard={false} centered>
                    <Modal.Header closeButton>
                        <Modal.Title>Módulo # {formik.values.id_modulo === 0 ? "nuevo" : formik.values.id_modulo}</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form>
                            <Row>
